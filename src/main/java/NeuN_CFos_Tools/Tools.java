...
import NeuN_CFos_Tools.StardistOrion.StarDist2D;
...
public class Tools {
    ...
    private final String helpUrl = "https://github.com/orion-cirb/NeuN_CFos.git";
    
    String[] chNames = {"NeuN", "CFos"};
    ...
    public double minNeunVol = 200;
    public double minCfosVol = 100;
    ...
    public String[] dialog(String imagesDir, String[] channels) {
        ...
        gd.addNumericField("Min NeuN volume (µm3): ", minNeunVol, 2);
        gd.addNumericField("Max CFos volume (µm3): ", minCfosVol, 2);
     ...
     */
   public Objects3DIntPopulation stardistDetection(ImagePlus img, double minCellVol, double maxCellVol) throws IOException{
       ...
       // Filter objects
       pop = new Objects3DIntPopulationComputation(pop).getExcludeBorders​(ImageHandler.wrap(imgIn), false);
       popFilterOneZ(pop);
    ...
    /**
     * Compute colocalization between pop1 and pop2
     */
    public ArrayList<Cell> colocalization(Objects3DIntPopulation neunPop, Objects3DIntPopulation cfosPop) {
        ArrayList<Cell> cells = new ArrayList<>();
        
        if (neunPop.getNbObjects() > 0 && cfosPop.getNbObjects() > 0) {
            MeasurePopulationColocalisation coloc = new MeasurePopulationColocalisation(neunPop, cfosPop);
            
            for (Object3DInt neun: neunPop.getObjects3DInt()) {
                Cell cell = new Cell();
                cell.setNeun(neun);

                List<PairObjects3DInt> list = coloc.getPairsObject1(neun.getLabel(), true);
                for (int i = list.size()-1; i >= 0; i--) {
                    Object3DInt cfos = list.get(i).getObject3D2();
                    if(cfos.getComment() != "coloc") {
                        double colocVol = list.get(i).getPairValue();
                        if ((colocVol > 0.5*neun.size()) || (colocVol > 0.5*cfos.size())) {
                            cfos.setComment("coloc");
                            cell.setCfos(cfos);
                            break;
                        }
                    }
            ...
            for (Object3DInt cfos: cfosPop.getObjects3DInt()) {
                if(cfos.getComment() != "coloc") {
                    Cell cell = new Cell();
                    cell.setCfos(cfos);
                    cells.add(cell);
     ...
     */
    public void computeParams(ArrayList<Cell> cells, ImagePlus imgNeun, ImagePlus imgCfos, double neunBg, double cfosBg) {
        ImageHandler imhNeun = ImageHandler.wrap(imgNeun);
        ImageHandler imhCfos = ImageHandler.wrap(imgCfos);
        
        double label = 1;
        for(Cell cell: cells) {
            cell.setParams(label, imhNeun, imhCfos, neunBg, cfosBg);
            label++;
        }
    }
     ...
     * Draw results
     */
    public void drawResults(ArrayList<Cell> cells, ImagePlus imgNeun, ImagePlus imgCfos, String name) {
        ImageHandler imhNeun = ImageHandler.wrap(imgNeun).createSameDimensions();
        ImageHandler imhCfos = imhNeun.createSameDimensions();
        
        // Draw NeuN pop in green and CFos pop in blue
        for (Cell cell: cells)  {
            int label = cell.getParams().get("label").intValue();
            if(cell.isNeun()) 
                cell.getNeun().drawObject(imhNeun, label);
            if(cell.isCfos())
                cell.getCfos().drawObject(imhCfos, label);
        }

        ImagePlus[] imgColors = {null, imhNeun.getImagePlus(), imhCfos.getImagePlus(), imgNeun, imgCfos};
        ...
        imhNeun.closeImagePlus();
        imhCfos.closeImagePlus();
    }
